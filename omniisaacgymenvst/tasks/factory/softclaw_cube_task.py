...
class SoftclawCubeTask(SoftclawCubeWS, FactoryABCTask):
    ...
    def reset_idx(self, env_ids):
        ...
        # close gripper to grasp the cube at the start
        for i in range(5):
            self._apply_actions_as_ctrl_targets(
    ...
    def _update_rew_buf(self):
        ...
        # if (self.level >= 4*self.cfg_task.rl.kickstart_max_level): # HACK bit hardcoded, to narrow the shape of the reward
        ...
        action_rate_pen = (
            torch.sum(torch.square(self.previous_actions - self.actions), dim=1) * 
                                    self.cfg_task.rl.action_rate_penalty_scale
        )
        self.rew_buf[:] += - action_rate_pen / self.max_episode_length
        self.episode_sums['action_rate_penalty'] += - action_rate_pen / self.max_episode_length
        ...
        if is_last_step:
            ...
            # else:
            self.rew_buf[:] = torch.where(dist_penalty < 0.08 * self.initial_dist, self.rew_buf[:] + 2000 * torch.ones_like(self.rew_buf), self.rew_buf)

            task_success = torch.where(dist_penalty < self._task_cfg["env"]["goal_zone_radius"] * self.initial_dist,
                                        torch.ones_like(dist_penalty), torch.zeros_like(dist_penalty))
            self.extras['task_success'] = torch.mean(task_success)
            self.rew_buf[:] += task_success * self.cfg_task.rl.task_success_scale
